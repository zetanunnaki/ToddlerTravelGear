...
export function ComparisonTable({
  ...
  return (
    <>
        ...
        {products.map((product) => (
          <div key={product.id} className="border border-gray-200 rounded-2xl p-5 bg-white shadow-sm">
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-4">
              {columns.map((col) => (
                <div key={col}>
                  <dt className="text-gray-400 text-xs">{columnLabels[col] || col}</dt>
      ...
      {/* Desktop: table */}
      <div className="my-8 overflow-x-auto hidden md:block rounded-2xl border border-gray-200">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-gray-50">
              <th className="text-left px-4 py-3 border-b border-gray-200 font-semibold text-gray-700 min-w-[140px]">
                Product
              </th>
              {columns.map((col) => (
                <th
              ...
              <th className="text-left px-4 py-3 border-b border-gray-200 font-semibold text-gray-700 min-w-[80px]">
                Price
              </th>
              <th className="px-4 py-3 border-b border-gray-200 min-w-[110px]" />
            </tr>
          </thead>
          <tbody>
            {products.map((product, i) => (
              <tr key={product.id} className={`hover:bg-teal-50/50 transition-colors ${i < products.length - 1 ? "border-b border-gray-100" : ""}`}>
                <td className="px-4 py-3 font-medium text-gray-900">
                  ...
                  <div className="text-xs text-gray-400 font-normal">{product.brand}</div>
                </td>
                {columns.map((col) => (
