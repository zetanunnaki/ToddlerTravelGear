...
<SafetyNote>
The Ingenuity 3D Mini meets applicable [CPSC stroller safety standards](https://www.cpsc.gov/Safety-Education/Safety-Guides/Kids-and-Babies/Strollers) and the [ASTM F833 standard](https://www.astm.org/f833-23.html) for carriages and strollers. Always use the 5-point harness when the child is seated. Lock the rear wheel brakes when the stroller is stationary. Do not exceed the weight limit (typically 50 lbs — check your specific model). Do not hang heavy bags on the handlebar, as this can tip the stroller backward. Ensure the fold lock is fully engaged before placing the child in the stroller. Check that the recline strap is secure before allowing the child to sleep reclined.
</SafetyNote>
...
<FAQ items={[
  ...
  { question: "Does the Ingenuity 3D Mini fit in an airplane overhead bin?", answer: "No, the folded size is too large for overhead bin storage. It is designed for gate-checking, which is free on all major airlines. The Stokke YOYO3 is the primary cabin-bag-compatible option." },
