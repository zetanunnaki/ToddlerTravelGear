...
<SafetyNote>
Check adhesion strength on each surface before leaving the child unattended — pull-test every lock after installation. Replace locks if the strap becomes loose or stretched. Do not rely solely on cabinet locks — remove the most dangerous items (cleaning chemicals, medications, sharp objects) to a high location if possible, as recommended by the [AAP's baby-proofing guidelines](https://www.healthychildren.org/English/safety-prevention/at-home/Pages/default.aspx). Cabinet locks are a barrier, not a guarantee — supervise toddlers in all unfamiliar environments. Test the lock release mechanism before your child does — ensure you can open it quickly in an emergency.
</SafetyNote>
